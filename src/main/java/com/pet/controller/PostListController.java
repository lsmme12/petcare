...
public class PostListController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<PostDTO> postList = dao.selectAll(); // 📋 모든 게시글 데이터를 가져옴 (List 형태로)
        // 💡 selectAll() 안에서는 “삭제 안 된 글만” 가져오도록 SQL이 작성돼 있어요.

        // ✅ 2️⃣ 가져온 게시글 목록을 JSP에서 쓸 수 있게 request에 담기
        request.setAttribute("postList", postList);
        // 💬 JSP에서는 ${postList} 로 접근해서 하나씩 출력할 수 있어요.

        // ✅ 3️⃣ list.jsp 페이지로 이동 (forward)
        // forward는 서버 안에서 페이지를 연결하는 방법이에요.
        // → 주소창은 그대로 /post/list.do 로 남아 있지만 실제 보여주는 건 list.jsp 화면이에요.
