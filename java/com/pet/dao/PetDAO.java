...
public class PetDAO extends DBConnPool {
    ...
    public PetDTO getPetById(int petId) {
        ...
        if (con == null) {
            System.err.println("[PetDAO] DB connection is null. Check Tomcat DataSource (JNDI name jdbc/myoracle) and context.xml");
            return null;
        }
    ...
    public List<PetDTO> getPetsByOwner(int ownerId) {
        ...
        if (con == null) {
            System.err.println("[PetDAO] DB connection is null. Cannot fetch pets by owner. Check DataSource jdbc/myoracle.");
            return list;
        }
