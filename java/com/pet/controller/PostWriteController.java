...
public class PostWriteController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // 🚫 원래 구조: 로그인이 안 되어 있으면 로그인 페이지로 보냄
        /*
        if (ownerId == null) {
        ...
        */
        ...
        // ✅ 테스트용: 로그인 안 되어 있으면 임시로 ownerId=1로 설정
        if (ownerId == null) {
            ownerId = 1; // ⚠️ OWNER 테이블에 실제 존재하는 유저의 owner_id 사용해야 함
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("👉 doPost 시작됨"); // 🔍 디버깅용 로그
        ...
        */
        ...
        // ✅ 폼 데이터 가져오기 (multipart form -> read Parts using UTF-8 to avoid encoding issues)
        String title = null;
        String content = null;
        try {
            Part titlePart = request.getPart("title");
            if (titlePart != null && titlePart.getSize() > 0) {
                try (InputStream is = titlePart.getInputStream()) {
                    title = new String(is.readAllBytes(), StandardCharsets.UTF_8).trim();
                }
            }
        } catch (Exception ignore) { /* fallthrough to getParameter */ }
        ...
        System.out.println("📄 title=" + title + ", content=" + content + ", ownerId=" + ownerId);
        ...
        if (filePart != null && filePart.getSize() > 0) {
            ...
            System.out.println("💾 파일 저장 완료: " + stored);
        }
        ...
        int result = dao.insertPost(dto); // 📦 DB의 POST 테이블에 새 글 추가
        dao.close(); // 🔒 DB 연결 닫기
        ...
        if (result > 0) {
            ...
            response.getWriter().println("<h3>게시글 등록 실패</h3>");
        }
    }
}